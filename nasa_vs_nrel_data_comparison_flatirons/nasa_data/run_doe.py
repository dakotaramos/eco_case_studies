...
if __name__ == "__main__":
    # build the model
    prob = om.Problem()
    prob.model.add_subsystem('hybrid_system', HybridSystem(location=flatirons_site, resource_api='nasa', battery=True, grid=True, sim_duration_years=25), promotes=['*'])

    # # setup the optimization
    prob.driver = om.ScipyOptimizeDriver()
    # prob.driver.options['optimizer'] = 'SLSQP'
    prob.driver = om.DOEDriver(om.FullFactorialGenerator(levels=1))
    prob.driver.options['debug_print'] = ["desvars", "objs"]
    prob.driver.add_recorder(om.SqliteRecorder("cases.sql"))
    prob.driver.recording_options['includes'] = ['*']
    
    # Solar DVs
    prob.model.add_design_var('solar_size_mw', lower=0.48, upper=0.48)
